...
export default function EmojiPicker({
  ...
  const categories = useMemo(() => {
    ...
    const availableCategories = Array.from(new Set(emojis.map((emoji) => emoji.category)));
  ...
  const emojisByCategory = useMemo(() => {
    return categories.reduce(
      (acc, category) => {
        acc[category] = filteredEmojis.filter((emoji) => emoji.category === category);
        return acc;
      },
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    switch (e.key) {
      case 'ArrowDown':
        e.preventDefault();
        setSelectedIndex((prev) => (prev < allVisibleEmojis.length - 1 ? prev + 1 : prev));
        break;
      case 'ArrowUp':
        e.preventDefault();
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="w-80 p-0" align="end" onKeyDown={handleKeyDown}>
        {/* Search Header */}
        ...
        {searchTerm ? (
          ...
          >
            ...
            >
              ...
              {categories.slice(0, recentEmojis.length > 0 ? 5 : 6).map((category) => {
                const IconComponent =
                  categoryIcons[category as keyof typeof categoryIcons] || Smile;
                return (
                  <TabsTrigger
                    key={category}
                    value={category}
